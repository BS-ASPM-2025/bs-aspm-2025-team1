...
async def jobs_manage_page(
    ...
    jobs = _job_service.list_job_summaries_for_company(db, company_id)
...
async def job_edit_page(
    ...
    job_description = ""
    if job.job_text:
        job_description = job.job_text.split(desc_marker, 1)[1] if desc_marker in job.job_text else job.job_text
